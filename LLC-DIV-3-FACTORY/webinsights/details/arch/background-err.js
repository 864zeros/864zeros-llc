        ...
        try {
          ...
          await chrome.scripting.executeScript({
            ...
            func: () => {
              ...
              function onMove(e) { 
                if (!isDown || !box) return;   // ✅ fine inside a function
...
// Centralized so background.js can enrich items consistently
// 9.10.25 JC > trying to {import} meta-data.js as a module caussed errors. module to be addressed later.
...
function mergeMetadataIntoItem(item, metadata) {
  ...
  return {
    ...
    metaKeywords: metadata.metaKeywords || item.metaKeywords || null,
...
async function handleSavePageContent() {
  ...
  // 1) Try content-script API first (preferred path)
  try {
    const resp = await chrome.tabs.sendMessage(tab.id, { type: "GET_PAGE_DATA" });
    if (resp?.success && resp.payload) {
      pd = resp.payload; // should already include meta fields from getPageData()
    } else {
    ...
    const [{ result }] = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        let text = "";
        try { text = doc.body?.innerText || ""; } catch {}
        let html = "";
        ...
        const links = Array.from(doc.body?.querySelectorAll("a[href]") || [])
          .map((a) => {
            ...
            if (!raw) return null;
            const low = raw.trim().toLowerCase();
            if (low.startsWith("#") || skip.some((p) => low.startsWith(p))) return null;
            try {
              const url = new URL(raw, baseURI).href;
  ...
  return await saveContent({
    ...
    pageDescription: pd?.metaDescription ?? null,
    pageKeywords: pd?.metaKeywords ?? null,
    host: pd?.host ?? null,
