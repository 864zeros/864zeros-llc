...
// Standard tier hierarchy (same across all 864zeros apps)
const TIER_LEVELS = {
  free: 0,
  starter: 1,
  pro: 2,
...
export function getFeatureTier(featureName) {
  return PRICING_CONFIG.features[featureName] ?? 'power';
}
...
/**
 * Get all features unlocked by upgrading from current tier to target tier.
 * @param {string} currentTier - Current tier
 * @param {string} targetTier - Target tier
 * @returns {string[]} Array of newly unlocked feature names
 */
export function getUnlockedFeatures(currentTier, targetTier) {
...
export function compareTiers(tierA, tierB) {
  const levelA = TIER_LEVELS[tierA] ?? 0;
  const levelB = TIER_LEVELS[tierB] ?? 0;
