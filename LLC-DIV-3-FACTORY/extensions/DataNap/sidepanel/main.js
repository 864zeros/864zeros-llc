// ============================================================
// MAIN — [OIA] TabVault Side Panel
// 864zeros LLC | OIA pillar | Slate & Sage palette
// UI logic, event handling, view switching.
//
// RULE-004 compliant: vault list uses BRK-UI-004 (accordion-record-v1)
// for the per-tab record cards. Header is click-to-toggle with primary
// "Restore Tab" action; body shows URL hot-link, vault metadata, and
...
// --- Render a single tab item as RULE-004 accordion-record ---
// Uses canonical .accordion-record-* class names so the BRK-UI-004 brick
// CSS + AccordionController behavior apply automatically. Header carries
// primary "Restore Tab" action (per directive #3); body has hot-link URL +
// vault metadata + secondary Restore + Delete actions.
function renderTabItem(tab) {
  const fallbackFavicon = 'data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 16 16%22><rect fill=%22%23888%22 width=%2216%22 height=%2216%22 rx=%222%22/></svg>';
  const favicon = tab.favIconUrl || fallbackFavicon;
  const titleEsc = escapeHtml(tab.title || '(untitled)');
  const urlEsc = escapeHtml(tab.url || '');
  const stamp = formatVaultedAt(tab.vaultedAt);
  const host = extractHost(tab.url);
  const hasScroll = (tab.scrollX > 0 || tab.scrollY > 0);
  const bodyId = `vault-record-body-${tab.id}`;

  return `
    <div class="accordion-record vault-record" data-id="${tab.id}">
      <div class="accordion-record__header"
           role="button" tabindex="0"
           aria-expanded="false"
           aria-controls="${bodyId}">
        <img class="vault-record__favicon"
             src="${favicon}"
             alt=""
             onerror="this.src='${fallbackFavicon}'"
             data-no-toggle />
        <div class="accordion-record__title-block">
          <div class="accordion-record__title" title="${urlEsc}">${titleEsc}</div>
          <div class="accordion-record__meta">${escapeHtml(stamp)}${host ? ' · ' + escapeHtml(host) : ''}</div>
        </div>
        <button class="vault-record__restore"
                type="button"
                data-no-toggle
                title="Restore Tab"
                aria-label="Restore tab: ${titleEsc}">
      ...
      <div class="accordion-record__body-wrapper">
        <div class="accordion-record__body" id="${bodyId}">
          <div class="accordion-record__reading">
            <p class="vault-record__url-row">
              <strong>URL:</strong>
              <a class="vault-record__hotlink" href="${urlEsc}" target="_blank" rel="noopener noreferrer" data-no-toggle>${urlEsc}</a>
            </p>
            <p class="vault-record__meta-row"><strong>Vaulted:</strong> ${escapeHtml(stamp)}</p>
            ${hasScroll ? `<p class="vault-record__meta-row"><strong>Scroll position:</strong> x=${tab.scrollX}, y=${tab.scrollY} (will be restored)</p>` : ''}
            ${tab.groupName ? `<p class="vault-record__meta-row"><strong>Session:</strong> ${escapeHtml(tab.groupName)}</p>` : ''}
          </div>
          <div class="accordion-record__actions">
            <a class="accordion-record__action"
               href="${urlEsc}" target="_blank" rel="noopener noreferrer"
               data-no-toggle>Open in new tab</a>
            <button class="accordion-record__action accordion-record__action--primary vault-record__restore-secondary"
                    type="button"
                    data-no-toggle>Restore Tab</button>
            <button class="accordion-record__action accordion-record__action--danger vault-record__delete"
                    type="button"
                    data-no-toggle>Delete</button>
          </div>
        </div>
      </div>
    </div>
...
function formatVaultedAt(timestamp) {
...
function attachVaultListeners(filteredTabs, sortedGroups) {
  // Per-record listeners (now accordion-records)
  vaultedTabsList.querySelectorAll('.vault-record').forEach(item => {
    const id = parseInt(item.dataset.id, 10);
    const tab = filteredTabs.find(t => t.id === id);
    if (!tab) return;

    // Header-primary Restore (RULE-004 §3 — present in header AND body)
    item.querySelector('.vault-record__restore')?.addEventListener('click', (e) => {
      e.stopPropagation();
      restoreTab(tab);
    });
    // Body-secondary Restore (redundant per directive #3)
    item.querySelector('.vault-record__restore-secondary')?.addEventListener('click', (e) => {
      e.stopPropagation();
      restoreTab(tab);
    });
    // Delete (body action row)
    item.querySelector('.vault-record__delete')?.addEventListener('click', (e) => {
      e.stopPropagation();
      deleteVaultedTab(id);
    });
    // "Open in new tab" link is a plain <a target="_blank"> — browser handles it.
  });

  // Re-sync accordion expansion state to the freshly-rendered DOM
...
document.addEventListener('DOMContentLoaded', () => {
  // Initialize accordion controller BEFORE first render so refresh()
  // picks up state correctly. Defaults: exclusive=true, shiftMultiExpand=true
  // (REQUIRED per RULE-004 — Compare Mode for ADHD-friendly focus).
  accordion = new AccordionController({
    container: vaultedTabsList,
  });
