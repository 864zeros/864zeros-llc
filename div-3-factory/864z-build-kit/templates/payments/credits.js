/**
 * Credits — 864zeros Payment Brick
 *
 * Usage-based credit system for AI and premium features.
 * Optional — only include if your app uses credits.
 *
 * Credits are stored in chrome.storage.local.
 */
...
/**
 * Check if credits system is enabled for this app.
 * @returns {boolean}
 */
 ...
 */
export async function resetCredits(amount = 0) {
  if (process.env.NODE_ENV === 'production') {
    console.warn('[credits] resetCredits blocked in production');
    return;
  }

  await chrome.storage.local.set({
    [STORAGE_KEY_CREDITS]: amount,
    [STORAGE_KEY_HISTORY]: [{
      type: 'reset',
      amount,
      timestamp: Date.now(),
